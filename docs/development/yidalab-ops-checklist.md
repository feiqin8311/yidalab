...
5. **任务定时计划**：进程内每 10 分钟扫一次（对齐原 QStash）；关掉用 `TASK_SCHEDULE_DISPATCH_CRON=0`。生产建议 `REDIS_URL` + `AGENT_RUNTIME_MODE=queue`。
6. **浏览器访问**：默认允许（账号登录）；若强制仅钉钉打开，设 `NEXT_PUBLIC_DINGTALK_ONLY=1`。
