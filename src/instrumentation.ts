import { shouldRunResidentWorkers } from '@lobechat/const/buildProfile';
...
export async function register() {
  ...
  // Single entry: do NOT also call startGateway from startServer.js.
  if (runWorkers && !process.env.VERCEL_ENV && process.env.ENABLE_BOT_IN_DEV !== '0') {
  ...
  // Daily memory analysis — explicit opt-in (MEMORY_DAILY_ANALYSIS_ENABLED=1).
  if (runWorkers) {
  ...
  // Use isScheduleDispatchCronEnabled (not !isTaskSchedulerV2On) so scoped V2
  // canary still runs legacy for out-of-scope workspaces.
  if (runWorkers) {
    void import('@/server/services/taskRunner/scheduleDispatchCron')
      .then(({ isScheduleDispatchCronEnabled, startScheduleDispatchCron }) => {
        if (isScheduleDispatchCronEnabled()) startScheduleDispatchCron();
      })
      .catch((err) => {
